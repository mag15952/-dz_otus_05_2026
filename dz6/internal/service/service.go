...
func NewGame(repo repository.Repo) *Game {
	return &Game{
		repo: repo,
	}
}

func (g *Game) SetGame(len, boardCount int, player1, player2 string) {
	g.players = models.NewPlayers(player1, player2)
	g.game = models.NewGame(boardCount)
	g.board = models.SetChessboard(len)

	// Сохраняем в репозиторий
	g.repo.SaveObject(player1, 0)
	g.repo.SaveObject(player2, 1)
	g.repo.SaveObject(g.game, 0)
	g.repo.SaveObject(g.board, 0)

}

func setCommandsNew(player string, newMove *models.MoveStruct, playerCount int) /*(bool, int, string)*/ {
...
func PlayChessNew(g *Game,
	player *models.Players, length int) {
...
func printAll(player *models.Players, allBoard [][]string) {
...
func makeMove(allBoard [][]string, m map[string]int,
	move *models.MoveStruct, g *Game) string {
